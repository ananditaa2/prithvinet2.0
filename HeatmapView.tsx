...
export default function HeatmapView() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      // Parse Noise by joining flat noise readings with monitoring locations.
    ...
    () =>
      airStations.find(
        (station) =>
          ...
          (station.name.toLowerCase().includes("jindal industrial park") ||
            station.name.toLowerCase().includes("punjipathra")),
      ) || null,
    [airStations],
  );
      ...
      airStations.forEach(s => {
        ...
        createRoot(popupEl).render(
          <PopupCard
            ...
            tone={evidence.tone}
            readings={[
              { label: evidence.metricLabel, value: evidence.metricValue },
              { label: evidence.limitLabel, value: evidence.limitValue },
            ]}
            footerLeft={getAirSourceLabel(s.source, s.note)}
            footerRight={formatDisplayDate(s.date)}
          />
        );
  ...
  useEffect(() => {
    ...
    map.flyTo([station.lat, station.lng], 11, {
      animate: true,
      duration: 1.2,
    });
      ...
      <div className="relative h-[650px]">
        <div className="absolute left-6 top-6 z-[1000] w-64 space-y-3">
          <div className="rounded-2xl border border-slate-700/70 bg-slate-950/80 p-4 backdrop-blur-xl">
            ...
            <Button
              size="sm"
              onClick={focusFeaturedViolation}
              disabled={!featuredAirStation}
              className="mb-3 w-full justify-start rounded-xl border border-red-400/25 bg-red-500/12 text-[11px] font-semibold text-red-100 hover:bg-red-500/18 disabled:opacity-50"
            >
